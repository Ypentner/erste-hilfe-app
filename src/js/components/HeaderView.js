import darkModeService from '../services/DarkModeService.js';

/**
 * Rendert den Dark Mode Toggle Button
 */
export function renderDarkModeToggle() {
  const isDark = darkModeService.getIsDark();
  
  return `
    <button id="darkModeToggle" 
            class="p-2 rounded-lg transition-colors hover:bg-slate-100 dark:hover:bg-slate-700"
            aria-label="Toggle Dark Mode">
      ${isDark ? `
        <!-- Sun Icon (Light Mode) -->
        <svg class="w-6 h-6 text-slate-700 dark:text-slate-200" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
                d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z" />
        </svg>
      ` : `
        <!-- Moon Icon (Dark Mode) -->
        <svg class="w-6 h-6 text-slate-700 dark:text-slate-200" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
                d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" />
        </svg>
      `}
    </button>
  `;
}

/**
 * Attach Dark Mode Toggle Listener
 */
export function attachDarkModeListener() {
  const toggleBtn = document.getElementById('darkModeToggle');
  
  if (toggleBtn) {
    toggleBtn.addEventListener('click', () => {
      darkModeService.toggle();
    });
  }
}